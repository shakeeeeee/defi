...
contract ZillaFinance is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 20000 ether;
    // Initial distribution for the day 2-5 ZILLA-WAVAX LP -> Zilla pool
    uint256 public constant INITIAL_ZILLA_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the Zilla ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Zilla Finance 2.0", "ZILLA") {
        // Mints 1 ZILLA to contract creator for initial pool setup
    ...
    function _getZillaPrice() internal view returns (uint256 _ZillaPrice) {
        try IOracle(ZillaOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Zilla: failed to fetch Zilla price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _ZillaPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_ZillaPrice >= taxTiersTwaps[tierId]) {
    ...
    function setZillaOracle(address _ZillaOracle) public onlyOperatorOrTaxOffice {
        require(_ZillaOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints ZILLA to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of ZILLA to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentZillaPrice = _getZillaPrice();
            currentTaxRate = _updateTaxRate(currentZillaPrice);
            if (currentZillaPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_ZillaPool != address(0), "!_ZillaPool");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_ZillaPool, INITIAL_ZILLA_POOL_DISTRIBUTION);
    }
